...
#           +-----------------------------+--------------------------+-----------------------+
#           |                             |                          |  - Simula la medicina |
#           |     simulate_medicine()     |         Ninguno          |    que le corresponde |
#           |                             |                          |    al adulto mayor.   |
#           +-----------------------------+--------------------------+-----------------------+
#           |                             |                          |  - Simula la dosis que|
#           |  simulate_medicine_dosis()  |         Ninguno          |    le corresponde al  |
#           |                             |                          |    adulto mayor.      |
#           +-----------------------------+--------------------------+-----------------------+
#
#-------------------------------------------------------------------------
...
class XiaomiMyBand:
    ...
    def publish(self):
        ...
        message = {}
        message['acceleration_x'] = self.simulate_x_position()
        message['acceleration_y'] = self.simulate_y_position()
        message['acceleration_z'] = self.simulate_z_position()
        message['id'] = str(self.id)
        message['datetime'] = self.simulate_datetime()
        message['producer'] = self.producer
        message['model'] = self.model
        ...
        print('[x] publicando valor de posicionamiento...')
        self.draw_progress_bar(2)
        channel.basic_publish(exchange='', routing_key='acceleration', body=str(message), properties=pika.BasicProperties(
            delivery_mode=2,))  # Se realiza la publicación del mensaje en el Distribuidor de Mensajes
        connection.close()  # Se cierra la conexión
        print('[x] valor publicado!')
        print('')
        ...
        message['delay'] = self.simulate_delay()
        message['medicine_dosis'] = self.simulate_medicine_dosis()
        message['id'] = str(self.id)
        message['datetime'] = self.simulate_datetime()
        message['producer'] = self.producer
        message['model'] = self.model
        ...
        print('[x] publicando medicamento...')
        self.draw_progress_bar(2)
    ...
    def simulate_medicine(self):
        medicine = ['parecetamol', 'ibuprofeno', 'insulina', 'furosemida', 'piroxicam', 'tolbutamida']
        return medicine[random.randint(0, len(medicine)-1)]


    def simulate_delay(self):
        return random.randint(0, 1)

    def simulate_medicine_dosis(self):
        return random.uniform(0, 100)


    def simulate_x_position(self):
        return random.uniform(-3, 13)
