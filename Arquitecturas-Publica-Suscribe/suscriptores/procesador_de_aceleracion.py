...
#           |     Procesador de     |    de posicionamiento   |    Xiaomi My Band.     |
#           |     Aceleracion       |    de los adultos       |  - Define los valores  |
#           |                       |    mayores.             |    de la posicion      |
#           |                       |                         |    actual de los       |
#           |                       |                         |    adultos mayores.    |
#           |                       |                         |    aceleracion.        |
#           |                       |                         |  - Notifica al monitor |
...
class ProcesadorAceleracion:
    ...
    def callback(self, ch, method, properties, body):
        ...
        if float(json_message['acceleration_x']) > 10 | float(json_message['acceleration_y']) > 2 | float(json_message['acceleration_z']) > 1:
            monitor = Monitor()
            monitor.print_notification('Fuera de la posicion habitual',json_message['datetime'], json_message['id'])
        time.sleep(1)
...
if __name__ == '__main__':
    procesador_de_aceleracion = ProcesadorAceleracion()
    procesador_de_aceleracion.consume()
