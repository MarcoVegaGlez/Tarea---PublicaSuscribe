...
#           |                       |                         |  - Se suscribe a los   |
#           |                       |                         |    eventos generados   |
#           |                       |  - Procesar el valor    |    por el wearable     |
#           |     Procesador de     |    de medicamento que   |    Xiaomi My Band.     |
#           |     Medicamentos      |    le toca a cada adulto|  - Define la medicina  |
#           |                       |    mayor.               |    correspondiente a   |
#           |                       |                         |    cada adulto mayor   |
#           |                       |                         |    ademas de su dosis. |
#           |                       |                         |  - Notifica al monitor |
#           |                       |                         |    cuando que medicina |
...
class ProcesadorMedicamentos:
    ...
    def consume(self):
        try:
            ...
            channel.basic_consume(self.callback, queue='medicine')
    ...
    def callback(self, ch, method, properties, body):
        ...
        if float(json_message['delay']) == 1:
            monitor = Monitor()
            monitor.print_notification('Fuera de tiempo',json_message['datetime'], json_message['id'])
        time.sleep(1)
...
if __name__ == '__main__':
    procesador_de_medicamentos = ProcesadorMedicamentos()
    procesador_de_medicamentos.consume()
